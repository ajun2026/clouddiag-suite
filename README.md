...
|---|---|---|
| Python 3.10+ | 运行服务 | — |
| **7-Zip** (`p7zip-full`) | `.7z` 解压（必需）；`.rar` 兜底解压器 | `.7z` 上传失败 | 
| **unrar** | `.rar` **首选**解压器（必需） | 缺 unrar 时 `.rar` 基本解不出（7z 对多数 RAR5 压缩方法报 Unsupported Method，会产出 0 字节文件） |
